        ...
        <TableBody>
          {events.map((event) => (
            <TableRow key={event.id}>
              ...
              <TableCell className="text-right tabular-nums">{event.photoCount}</TableCell>
              <TableCell>
              ...
              <TableCell>
                <div className="flex flex-wrap justify-end gap-1.5">
                  <Button asChild variant="outline" size="sm">
                    <Link href={`${ADMIN_PATHS.photos}?event=${event.id}`}>Fotos</Link>
                  </Button>
                  <EventEditDialog event={event} />
                  <form action={setEventActiveAction}>
                    <input type="hidden" name="eventId" value={event.id} />
                    <input type="hidden" name="isActive" value={event.isActive ? "false" : "true"} />
